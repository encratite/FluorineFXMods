...
namespace FluorineFx
{
	...
	internal class __Res
	{
		...
		internal static string GetString(string key)
		{
			if (_resMgr == null)
			{
                _resMgr = new ResourceManager("FluorineFx.WCF.Resources.Resource", typeof(__Res).Assembly);
			}
			string text = _resMgr.GetString(key);
			if (text == null)
			{
                throw new Exception("Missing resource from FluorineFx.WCF library!  Key: " + key);
			}
			return text;
		}
